...
addpath('/home/users/fangq/space/git/Project/github/iso2mesh');
addpath('/home/users/fangq/space/git/Project/jsonlab');
addpath('/home/users/fangq/space/git/Project/github/jnifty');
...
nii = loadnifti(fullfile(hm, 'mask_skin.nii')); h = nii.NIFTIHeader;
A = h.Affine; if (size(A, 1) == 3); A = [A; 0 0 0 1]; end
scalp = (A(1:3, 1:3) * sv' + A(1:3, 4))';

% weighting (alpha=0.003, beta=0.01) and inversion
alpha = 0.003; beta = 0.01;
wrow = 1 ./ sqrt(cmeas + 1e-6 * max(cmeas));
Aw = wrow .* Anorm; cn = sqrt(sum(Aw .^ 2, 1))'; Wc = 1 ./ (cn + beta * max(cn));
Aw = Aw .* Wc'; lam = alpha * max(sum(Aw .^ 2, 2));
...
for j = 1:3
    ...
    z = rbreginv(Aw, wrow .* rhs, lam); d = z .* Wc;
    scalp_h = d(ncortex + (1:nscalp));            % scalp-surface portion
    subplot(1, 3, j);
    plotmesh([scalp, scalp_h(:)], sf(:, 1:3), 'edgecolor', 'none', 'facecolor', 'interp');
    view([0 90]); axis equal off; camlight; lighting gouraud;
    mm = max(abs(scalp_h)); if (mm == 0); mm = 1; end; caxis([-mm mm]); colormap(cmap); colorbar;
