...
if (nargin < 1 || isempty(alpha)); alpha = 0.01; end
...
wl = double(meas.wavelengths(:))'; wlkey = arrayfun(@(x) num2str(x), wl, 'UniformOutput', false);
...
npair = size(pairs, 1); nwl = numel(wl);
...
if (exist(cachef, 'file'))
    ...
    S = load(cachef);
    Anorm = S.Anorm; cmeas = S.cmeas; ncortex = S.ncortex; nscalp = S.nscalp;
else
    mesh = loadjmesh(fullfile(here, 'head_mesh.bmsh'));
    node = meshpart(mesh.MeshVertex3); [etet, elabel] = meshpart(mesh.MeshTet4);
    elem = [etet, elabel]; nseg = max(elem(:, 5));
    ...
    clear cfg; cfg.node = node; cfg.elem = elem; cfg.seg = elem(:, 5);
    cfg.srcpos = srcpos; cfg.srcdir = srcdir; cfg.detpos = detpos; cfg.detdir = detdir;
    %            scalp bone CSF  gray white
    tis.hbo = [25 15 2 60 40]; tis.hbr = [12 8 1 25 18];
    tis.scatamp = [1.2 1.35 0.4 1.65 1.5]; tis.scatpow = [1 1 1 1 1];
    ...
    for w = 1:nwl; cfg.prop(wlkey{w}) = [0 0 1 1; repmat([0.015 1 0 1.37], nseg, 1)]; end
    cfg.omega = 0; cfg = rbmeshprep(cfg);

    ns = size(srcpos, 1); nd = size(detpos, 1);
    sdfull = rbsdmap(cfg);
    kf = sdfull.keys; for i = 1:numel(kf); t = sdfull(kf{i}); t(:, 3) = 1; sdfull(kf{i}) = t; end
    [detphi_base, phi] = rbrunforward(cfg, 'sd', sdfull);
    if (isa(phi, 'containers.Map')); phimap = phi; else; phimap = phi(1).phi; end
    ...
    Js = containers.Map();
    for w = 1:nwl
        cfgw = cfg; cfgw.prop = cfg.prop(wlkey{w});
...
for j = 1:size(targets, 1)
    ...
    for w = 1:nwl; rhs((w - 1) * npair + (1:npair)) = -targets{j, 2}(:, w); end
    z = rbreginv(Aw, wrow .* rhs, lambda); d = z .* Wc;
    h = d(1:Nsurf);
    hbo(targets{j, 1}) = h;
    hbr(targets{j, 1}) = d(Nsurf + (1:Nsurf));
    cortex_h = h(1:ncortex); scalp_h = h(ncortex + (1:nscalp));
...
cidx = 1:ncortex;
hboC3 = hbo('C3'); hbrC3 = hbr('C3'); hboC4 = hbo('C4'); hbrC4 = hbr('C4');
...
n = 256; tt = linspace(0, 1, n)'; cmap = [min(1, 2 * tt), 1 - abs(2 * tt - 1), min(1, 2 * (1 - tt))];
...
for i = 1:6
    val = panels{i, 1}; val = val(1:ncortex);
    subplot(2, 3, i);
    plotmesh([cortex, val(:)], face_c, 'edgecolor', 'none', 'facecolor', 'interp');
    view(panels{i, 3}); axis equal off; camlight; lighting gouraud;
    mm = max(abs(val)); if (mm == 0); mm = 1; end; caxis([-mm mm]); colormap(cmap); colorbar;
...
val = [];
if (isstruct(x)); arr = double(x.Data);
    if (isfield(x, 'Properties') && isfield(x.Properties, 'Value')); val = double(x.Properties.Value); end
else; arr = double(x); end
end
function k = detkey(m, ck)
if (isKey(m, ck)); k = ck; else; k = str2double(ck); end
end
function [p, dir] = snap2surf(node, face, nv, p)
si = unique(face(:)); k = nearestnodes(node(si, :), p); ci = si(k);
dir = -nv(ci, :); p = node(ci, :) + 1.0 * dir;
end
function idx = nearestnodes(node, q)
if (exist('knnsearch', 'file')); idx = knnsearch(node, q); return; end
idx = zeros(size(q, 1), 1);
for i = 1:size(q, 1); d = node - q(i, :); [~, idx(i)] = min(sum(d .* d, 2)); end
end
function A = getaffine(nii)
h = nii.NIFTIHeader;
if (isfield(h, 'Affine') && ~isempty(h.Affine)); A = h.Affine; if (size(A, 1) == 3); A = [A; 0 0 0 1]; end
else; A = eye(4); end
end
function addtoolboxpaths()
r = '/home/users/fangq/space/git/Project';
c = {fullfile(r, 'github', 'redbird-m', 'matlab'), fullfile(r, 'github', 'iso2mesh'), ...
     fullfile(r, 'jsonlab'), fullfile(r, 'github', 'jnifty'), fullfile(r, 'github', 'jmesh', 'lib', 'matlab')};
for i = 1:numel(c); if (exist(c{i}, 'dir')); addpath(c{i}); end; end
end
