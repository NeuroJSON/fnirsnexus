...
if (nargin < 1 || isempty(peaktime)); peaktime = 10.0; end
if (nargin < 2 || isempty(alpha));    alpha = 0.01;    end
if (nargin < 3 || isempty(beta));     beta  = 0.01;    end
...
if (isempty(elabel)); elabel = ones(size(etet, 1), 1); end
elem = [etet, elabel];
nseg = max(elem(:, 5));
...
if (exist(cachef, 'file'))
    ...
    S = load(cachef);
    Anorm = S.Anorm; rnode = S.rnode; nrec = S.nrec; npair = S.npair;
    nwl = S.nwl; widx = S.widx;
else
...
clear cfg;
cfg.node = node;  cfg.elem = elem;  cfg.seg = elem(:, 5);
cfg.srcpos = srcpos;  cfg.srcdir = srcdir;
cfg.detpos = detpos;  cfg.detdir = detdir;
...
ns = size(srcpos, 1);
sdfull = rbsdmap(cfg);
k = sdfull.keys; for i = 1:numel(k); t = sdfull(k{i}); t(:, 3) = 1; sdfull(k{i}) = t; end
...
Jr = containers.Map();
for w = 1:numel(wlkey)
    cfgw = cfg; cfgw.prop = cfg.prop(wlkey{w});           % single-wl 2-row prop
...
nwl   = numel(canon);
npair = size(pairs, 1);
ck = cell(1, nwl); widx = zeros(1, nwl);
for w = 1:nwl
    if (ischar(canon{w})); ck{w} = canon{w}; else; ck{w} = num2str(canon{w}); end
...
if (isKey(m, charkey)); k = charkey; return; end
nk = str2double(charkey);
if (isKey(m, nk)); k = nk; return; end
...
sd = containers.Map();
for w = 1:numel(wlkey); sd(wlkey{w}) = tab; end
end
...
cand = {fullfile(root, 'github', 'redbird-m', 'matlab'), ...
        fullfile(root, 'github', 'iso2mesh'), ...
        fullfile(root, 'jsonlab'), ...
        fullfile(root, 'github', 'jnifty'), ...
        fullfile(root, 'github', 'jmesh', 'lib', 'matlab')};
for i = 1:numel(cand)
    if (exist(cand{i}, 'dir')); addpath(cand{i}); end
end
end
