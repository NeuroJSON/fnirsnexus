...
node = double(gt.GIFTIData.MeshVertex3.Data); face = double(gt.GIFTIData.MeshTri3.Data);
G = gt.GIFTIData.MeshVertex3.Properties; R = rc.GIFTIData.MeshVertex3.Properties;
...
for i = 1:4
    ...
    view(P{i,3}); axis equal off; camlight; lighting gouraud;
    cm = P{i,4}; if (cm == 0); cm = max(abs(P{i,1})); if (cm == 0); cm = 1; end; end
    caxis([-cm cm]); colormap(cmap); colorbar;
