...
clear cfg;
cfg.node = node;
cfg.elem = [elem(:, 1:4), ones(size(elem, 1), 1)];
...
for w = 1:numel(wl)
    cfg.prop(wlkey{w}) = [0 0 1 1; 0.01 1.0 0 1.37];
end
cfg.omega = 0;
cfg = rbmeshprep(cfg);
...
gmnodes = unique(gmel);
newidx = zeros(Nn, 1);
newidx(gmnodes) = 1:numel(gmnodes);
clear recon;
...
node_c = double(gt.GIFTIData.MeshVertex3.Data);
